...
def test_prep():
    ...
    if False:
        ...
        a = np.random.normal(means, variances, size=(n_cells, n_genes)).astype(
            np.float32
        )
        ...
        adata = anndata.AnnData(
            a, var=pd.DataFrame({"hgnc_symbol": genenames}).set_index("hgnc_symbol")
        )
    else:
        # import scanpy as sc
    ...
    np.testing.assert_allclose(
        adata_observed.X.toarray(), adata_expected.X, rtol=0.0002
    )
